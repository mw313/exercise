...
<tr>
  <td colspan="<?php echo $fromExercise?2:3; ?>" align="right">
	...
	<option value="-1"  selected="selected">-- <?php echo get_lang('OrphanQuestions'); ?> --</option>
<?php //if($exerciseId == -1) echo 'selected="selected"'; ?> 
<?php 
...
<?php
	...
	elseif($exerciseId == -1)
	{
		...
		$sql="SELECT $TBL_QUESTIONS.id  , question, $TBL_QUESTIONS.type, exercice_id FROM $TBL_QUESTIONS 
		 LEFT JOIN $TBL_EXERCICE_QUESTION  ON $TBL_QUESTIONS.id=$TBL_EXERCICE_QUESTION.question_id left join 
			$TBL_EXERCICES on ($TBL_EXERCICES .id=$TBL_EXERCICE_QUESTION.exercice_id )
			where $TBL_EXERCICES .id is NULL";
	}
	...
	$sql="SELECT $TBL_QUESTIONS.id  , question, $TBL_QUESTIONS.type, exercice_id FROM $TBL_QUESTIONS 
		 LEFT JOIN $TBL_EXERCICE_QUESTION  ON $TBL_QUESTIONS.id=$TBL_EXERCICE_QUESTION.question_id left join 
			$TBL_EXERCICES on ($TBL_EXERCICES .id=$TBL_EXERCICE_QUESTION.exercice_id )
			where $TBL_QUESTIONS.type=5";
// $TBL_EXERCICES .id is NULL
	$result=api_sql_query($sql,__FILE__,__LINE__);
