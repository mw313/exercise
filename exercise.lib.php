...
echo " <script>
 function textCounter(area_id, cunter, maxlimit) {
  field = document.getElementById(area_id)
  if (field.value.length > maxlimit)
   field.value = field.value.substring(0, maxlimit)
  else 
   document.getElementById(cunter).innerHTML = maxlimit - field.value.length
  }
 </script>";
    ...
    //for($i=0;$i<8;$i++)
		if(!$onlyAnswers)
		{
			...
			//*********** change by khakbazzadeh
			$questionName= strip_tags(api_parse_tex($questionName),"<p><div><br /><br><a><b><br/><img><img /><table></table><tr></tr><td></td>");
			//***********************************
			$s.=$questionName;
			$s.="</td>
			</tr>
		...
		elseif($answerType == FREE_ANSWER)
		{
			...
			$upload_path = api_get_path(REL_COURSE_PATH).$_SESSION['_course']['path'].'/document/images/user_'.$_SESSION['_user']['user_id']."/";
			// if(is_dir()){
				// disk
			// }
			// echo 123;
			...
			// $oFCKeditor->Config['IMUploadPath'] = $upload_path;
			// $oFCKeditor->ToolbarSet = "Basic";
			$oFCKeditor->Width  = '80%';
			$oFCKeditor->Height = '300';
			$oFCKeditor->Value	= '' ;
			// $oFCKeditor->Config['ImageBrowserURL'] = $oFCKeditor->BasePath . "editor/filemanager/browser/default/browser.html?Type=Video&Connector=connectors/php/connector.php&ServerPath=$upload_path";
			// $oFCKeditor->Config['ImageUploadURL'] = $oFCKeditor->BasePath . "editor/filemanager/upload/php/upload.php?Type=Video&ServerPath=$upload_path" ;
			...
			// By ghanbari disable fck editor on qiuzes and using text area instead of it
			// $s .= "<tr><td colspan='2'>".$oFCKeditor->CreateHtml()."</td></tr>";
			$class = "";
			if(in_array($_GET['cidReq'], array('18402', '14406'))){
				$class = 'editor';
			}
				
			?>
				(<b id="count<?echo $questionId ?>"><?php echo Free_Answer_Character; ?></b> &#1705;&#1575;&#1585;&#1575;&#1705;&#1578;&#1585; &#1576;&#1575;&#1602;&#1740; &#1605;&#1575;&#1606;&#1583;&#1607;)<br>

			<?
				$s.="<tr><td colspan='2'>
						<textarea cols='80' class='$class' id='txt[".$questionId."]' rows='10' name='choice[".$questionId."]' onKeyUp="." \""."textCounter('txt[".$questionId."]','count".$questionId."',".Free_Answer_Character.");"."\""." onKeyDown="."\""."textCounter('txt[".$questionId."]','count".$questionId."',".Free_Answer_Character.");"."\""." onchange="."\""."textCounter('txt[".$questionId."]','count".$questionId."',".Free_Answer_Character.");"."\"".">$answer</textarea>	
					</td></tr>";
	...
	echo "<script src='http://www.ikvu.ac.ir/main/inc/lib/tinymce_5.2.1/tinymce/js/tinymce/tinymce.min.js'></script>";
	echo "<script>			
			tinymce.init({
				// selector: '#mytextarea',
				selector: '.editor',
