...
// this code adde by ghanbari to prevent student multi answering
// Disabled at aban 91
// enabled again at ordibehesht 91
...
$sql1 = "select * from `current_semester`";
$q1 = mysql_query($sql1);
$d1 = mysql_fetch_array($q1) or die(mysql_error());
//echo $d1['test_excercise']."IS A ";
//mysql_close($con1);
if($d1['only_one_reply'] == 1)
{
    //exit();
    mysql_select_db("dokeos_stats", $con);
    mysql_query("SET NAMES 'utf8'", $con);

    $sql="select * from dokeos_stats.track_e_exercices where exe_user_id= ".api_get_user_id(). " and exe_cours_id= ". $_GET['cidReq']. " and exe_exo_id= ". $_GET['exerciseId'];
    //echo $sql;
    //exit();
    ...
    if ($Flag == 0)
    {
        //header("location:exercise_show.php?id=$id");
        header("location:exercise_show.php?id=$id");
    }
}
...
else
{
    ...
    //if()
    //$onsubmit = "onsubmit = 'return checkEmpty()'";
	...
	if($origin == 'learnpath' && $exerciseType==2){
		...
		$s2 = "&exerciseId=".$exerciseId."&cidReq=".$_GET["cidReq"];
	...
	$s="</table>
	 ...
	 <tr>
	  <td>
       ...
       function message()
       {
            return confirm('پاسخ سوالات ارسال شود؟\\n پس از ثبت پاسخ ها دیگر قادر به ویرایش آن نیستید!');
       }
       function message2()
       {
            ...
            {
              tVal += textareaList[i].value;
            }

            //alert(tVal);
            if(tVal == '')
            {
                alert('لطفا پاسخ های خود را وارد نمایید!!');
                return false;
            }
            else
            {
       ...
       function checkEmpty()
       {
         ...
         $q = Database::query("select * from dokeos_main.current_semester");
         $d = mysql_fetch_array($q);

	  if ($exerciseType == 1 || $nbrQuestions == $questionNum)
	  {
	    $func = "message2";
	    if($d['only_one_reply'] == 1)
            $func = "message";
		$s.=get_lang('ValidateAnswer').'\' onclick="return '.$func.'();" />';
	  }
	  else
	  {
		$s.=get_lang('Next').' &gt;\' />';
