...
abstract class Question
{
    ...
    function saveMulti($exerciseId=0 , $counter)
	{
		global $TBL_QUESTIONS, $TBL_EXERCICE_QUESTION, $_course;
        
        $suffix = "_".$counter;
		//$id=$this->id;
		$question=addslashes($_POST['questionName'.$suffix]);        
		$description=addslashes($_POST['questionDescription'.$suffix]);
		$weighting=addslashes($_POST['weighting'.$suffix]);
		$type=$this->type;
		$picture=addslashes($this->picture);
        ...
        # echo("<div style='direction:ltr;'>".$sql." & ID: ".$exerciseId."</div>");
		$this->id = mysql_insert_id();
    ...
    function createForm (&$form , $i = "no") 
    {
        
		echo '<style>
				div.row div.label{ width: 10%; }
				div.row div.formw{ width: 89%; }
                div.type { text-align: center; margin:10px 0px; }
			</style>
            ';
		
		
        # Codes For multi questions
        $suffix = "";
        $label = "";
        $theme_start = "<div class='odd'>";
        ...
        if($_GET['type']=="editor")
        {
    		...
    		$form->add_html_editor('questionName'.$suffix, get_lang('QuestionText'), false);
        }
        else
        {
            $gets = array();
            ...
            $gets[]="type=editor";
            $atitle = "استفاده از ادیتور";
            
            # question name
            $test=$form->addElement('textarea','questionName'.$suffix,get_lang('QuestionText').$label0,'size="110" cols="80"');
        }
        if($i==1 || $i=="no")
		echo "<div class='type'> <a href='admin.php?".implode("&",$gets)."'>$atitle</a></div>";
		// echo "<div class='type'> <a target='_blank' href='http://ikvu.ac.ir/main/exercice/uploader/elFinder-2.1.53/upload.php'>آپلودر</a></div>";
        //$renderer = $form->defaultRenderer();
		//$renderer->setElementTemplate('<div class="row"><div class="label">{label}</div><div class="formw">{element}</div></div>','questionName');
             
        
        # $form -> setDefaults(array('questionName'.$suffix => get_lang('QuestionText').$label));
		$renderer = $form->defaultRenderer();
		$renderer->setElementTemplate($theme_start.'<div class="row rightside"><div class="label sidelabel">{label}</div><div class="formw">{element}</div></div>','questionName'.$suffix);
		# $form->addRule('questionName'.$suffix, get_lang('GiveQuestion'), 'required');
		...
		$form->add_html_editor('questionDescription'.$suffix, get_lang('QuestionDescription'), false);
		$renderer->setElementTemplate('<div class="row"><div class="label">{label}</div><div class="formw">{element}</div></div>','questionDescription');
        */
        
		// hidden values
		$form->addElement('hidden','myid'.$suffix,$_REQUEST['myid']);
    ...
    {
        # Changed By Mahdi W - Manage Multi Question
        $mod = $_POST['answerType'];
        if($_GET['newQuestion'] == "yes" && ($mod == 1 || $mod == 5))
        {
            if($mod == 5)
            ...
            for($i = 1; $i <= $n; $i++)
            {
                $this -> saveMulti($objExercise -> id , $i);
                $objExercise->addToList($this -> id);
                $objExercise->save();
                
                if($mod == 1)
                $this -> MyProcessAnswersCreation($form,$i);
            }
            # exit();
        }
        # For Single Question
