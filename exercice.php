...
// by ghanbari to limit user access that haves shift=2
$u_info= api_get_user_info(api_get_user_id());
$shift=$u_info['shift'];
if ($shift==2)
 header('Location: http://www.ikvu.ac.ir/user_portal.php');
...
$is_allowedToEdit = api_is_allowed_to_edit();
 	//added by vahid ghanbari connetion 2 db for qualify user access in tools
		$con = mysql_connect("mysqlserver","root","qwer$#@!QWER");
		mysql_select_db("dokeos_main",$con);
		$access_sql="select * from access_rel_user where user_id = " . api_get_user_id();
		$result=mysql_query($access_sql);
		$user_access=mysql_fetch_array($result);
		if ($user_access['quiz']!=1)
...
<table class="data_table">
	...
	if ($origin != 'learnpath') 
	{
		...
		while($row=Database::fetch_array($result))
		{
		  ...
		  echo $rowi.' '.strtolower(get_lang(($rowi>1?'Questions':'Question'))).'</td>';
	       ...
	       <a href="exercise_admin.php?modifyExercise=yes&exerciseId=<?php echo $row['id']; ?>#exerciseId<?php echo $row['id']; ?>" name="exerciseId<?php echo $row['id']; ?>"> <img src="../img/edit.gif" border="0" title="<?php echo htmlentities(get_lang('Modify'),ENT_QUOTES,$charset); ?>" alt="<?php echo htmlentities(get_lang('Modify'),ENT_QUOTES,$charset); ?>" /></a>
	       <a href="admin.php?exerciseId=<?php echo $row['id']; ?>#exerciseId<?php echo $row['id']; ?>"><img src="../img/wizard_small.gif" border="0" title="<?php echo htmlentities(get_lang('Build'),ENT_QUOTES,$charset); ?>" alt="<?php echo htmlentities(get_lang('Build'),ENT_QUOTES,$charset); ?>" /></a>
	       <a href="exercice.php?choice=delete&exerciseId=<?php echo $row['id']; ?>#exerciseId<?php echo $row['id']; ?>" onclick="javascript:if(!confirm('<?php echo addslashes(htmlentities(get_lang('AreYouSureToDelete'),ENT_QUOTES,$charset)); echo " ".$row['title']; echo "?"; ?>')) return false;"> <img src="../img/delete.gif" border="0" alt="<?php echo htmlentities(get_lang('Delete'),ENT_QUOTES,$charset); ?>" /></a>
	    <?php
				// if active
				if($row['active'])
				{
					?>
      <a href="exercice.php?choice=disable&page=<?php echo $page; ?>&exerciseId=<?php echo $row['id']; ?>#exerciseId<?php echo $row['id']; ?>"> <img src="../img/visible.gif" border="0" alt="<?php echo htmlentities(get_lang('Deactivate'),ENT_QUOTES,$charset); ?>" /></a>
    <?php
				}
				// else if not active
				else
				{
					?>
      <a href="exercice.php?choice=enable&page=<?php echo $page; ?>&exerciseId=<?php echo $row['id']; ?>#exerciseId<?php echo $row['id']; ?>"> <img src="../img/invisible.gif" border="0" alt="<?php echo htmlentities(get_lang('Activate'),ENT_QUOTES,$charset); ?>" /></a>
    <?php
				}
  ...
  echo ($rowi>1?get_lang('Questions'):get_lang('Question')); ?> </td>
	...
	$qry = "select * from $TBL_TRACK_EXERCICES where exe_exo_id = $eid and exe_user_id = $uid and exe_cours_id = '".api_get_course_id()."' ORDER BY exe_id ";
	...
	if($show == 'result')
	{
		...
		<table class="data_table">
		 <tr class="row_odd">
          ...
          <th>ردیف</th>
		  <th>شماره دانشجویی <?php //echo get_lang("User"); ?></th>
		  <th> نام و نام خانوادگی <?php //echo get_lang("User"); ?></th><?php endif; ?>
		...
		if($is_allowedToEdit || $is_tutor)
		{
			...
			//changed by ghanbari to show last answer of std to teacher	
			if (api_get_user_id()==1)
				$sql="SELECT CONCAT(lastname,' ',firstname),ce.title, te.exe_result ,
						te.exe_weighting, UNIX_TIMESTAMP(te.exe_date),te.exe_id,email,username
				  ...
				  ORDER BY te.exe_date asc,te.exe_cours_id ASC, ce.title ASC";				

			else 
			$sql=" select * from ( SELECT CONCAT(lastname,' ',firstname),ce.title, te.exe_result ,
						te.exe_weighting, UNIX_TIMESTAMP(te.exe_date),te.exe_id,email,username
				  FROM $TBL_EXERCICES AS ce , $TBL_TRACK_EXERCICES AS te, $TBL_USER AS user
				  WHERE te.exe_exo_id = ce.id AND user_id=te.exe_user_id AND te.exe_cours_id='$_cid'
				  ORDER BY UNIX_TIMESTAMP(te.exe_date) desc,te.exe_date asc,te.exe_cours_id ASC, ce.title ASC) as tbl
				 group by  tbl.username, tbl.title";
			...
			$hpsql="SELECT CONCAT(tu.lastname,' ',tu.firstname), tth.exe_name,
						tth.exe_result , tth.exe_weighting, UNIX_TIMESTAMP(tth.exe_date),username
		...
		if(is_array($results))
		{
			for($i = 0; $i < sizeof($results); $i++)
			{
				...
				$mailid = $results[$i][6];
								//by ghabbari
				$username=$results[$i][7];

				$user = $results[$i][0];
				$user=@iconv('windows-1256','utf-8',$user);
				$test = $results[$i][1];
				$dt = strftime($dateTimeFormatLong,$results[$i][4]);
				$dt=@iconv('windows-1256','utf-8',$dt);
				$res = $results[$i][2];
				//echo $results[$i][0].":".$results[$i][2]."<br / >";
				//added by ghanbari to color corrected rows
				if ($res==0)
					$color="";
					//$color=" bgcolor='#6699FF' ";
				else
					$color=" bgcolor='#6699FF' ";
				echo '<tr id="td'.$id.'" ';
				echo $color;
				...
				echo '<td>'.(($is_allowedToEdit||$is_tutor)?"<a href='exercise_show.php?dt=$dt&res=$res&id=$id&email=$mailid&user=$user'>".get_lang("Edit")."</a>":"<a href='exercise_show.php?dt=$dt&res=$res&id=$id'>".get_lang('Show')."</a>").'</td>';
				echo '</tr>';
			}
		}
		else
...
if($info['EduSecCode']==1||$info['EduSecCode']==4||$info['EduSecCode']==5)
 { 
?>
...
<script>
//TINY.box.show({iframe:'tiny_text.htm',boxid:'frameless',width:800,height:750,fixed:false,maskid:'bluemask',maskopacity:40,closejs:function(){closeJS()}})
</script>
