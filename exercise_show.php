...
//functions
function getUserInfo($user_name , $emailId)
{
    $user_info = "";
    $user_info = $user_name ." : ".$emailId;
    return $user_info;
}
...
function display_unique_or_multiple_answer($answerType, $studentChoice, $answer, $answerComment, $answerCorrect,$id,$questionId,$ans)
{
	...
	<td width="45%" style="font-size: 17px;">
		<?php
		$answer=api_parse_tex($answer);
		echo $answer; 
        ?>
        <hr  style="border-top: 0.5px solid #4171B5;">
	</td>
	<?php if($ans==1)
...
function display_free_answer($answer,$id,$questionId)
{
    ...
    <tr>
		<td>
            ...
            background-color:#FFF; text-align: justify; padding: 15px 25px;
            ">'.nl2br(strip_tags(stripslashes($answer), "<p><div><br /><br><a><b><br/><img><img />")).'</div>
        </td>
    ');
    if(!$is_allowedToEdit) 
    {
...
==============================================================================
*/
//echo $_SESSION['_user']['status'];
if($_SESSION['_user']['status'] == 1)
{
    echo('
    <div style="background-color:#FFF; padding:15px;border-radius:7px; width:95%; margin:2px auto; border:1px solid #e0e0e0;">
        <b style="font-family: tahoma; font-size: 11px; font-weight: bold; color:red; text-align: center; margin-right: 5px; line-height:170%;">
        <span style="color: blue;">استاد گرامي :</span>
        تعداد سوالات میان ترم 6 سوال می باشد که بارم آن در پيش فرض سيستم براي هر سوالي امتیاز لحاظ شده  است: امتیاز 4 معادل یک نمره، امتیاز 3 معادل هفتادو پنج صدم، امتیاز 2 معادل نیم نمره، امتیاز 1 معادل بیست و پنج صدم می باشد. پس از تصحیح روي گزينه تاييد کليک کنيد. در هر صورت گزینه تایید باید کلیک شود.
        </b>
    </div>
    ');
}
...
?>
	...
	<table>
		...
		<tr>
			...
			<td><?php echo getUserInfo($user_name , $emailId); ?></td>
		</tr>
		<tr>
 ...
 if($debug>0){echo "ExerciseResult: "; var_dump($exerciseResult); echo "QuestionList: ";var_dump($questionList);}
  ...
  <tr>
    <td colspan="2" > 
        <div style="line-height: 40px; padding-right: 15px; background-color: #2694ED; border-radius: 5px; 
        margin: 2px auto; width:96%; color: #2E5C5C; color: #FFF; font-size: 15px; border: 1px solid #1287D6;">
            <?php echo get_lang("Question").' '.($counter).' : '.strip_tags(api_parse_tex($questionName),"<p><div><br /><br><a><b><br/><img><img />");; ?> 
        </div> 
    </td>
  </tr>
   <tr>
  ...
  <tr>
  <td width="550" height="90" valign="top">
  <?php
				 ...
				 if($is_allowedToEdit)
						{
							if ($answerType == FREE_ANSWER)
							 ...
							 <form name="marksform_<?php echo $questionId; ?>" method="post" action="">
                                  ...
                                  //echo('$questionScore'.$questionScore);
								  ...
								  for($i=0;$i<=$questionWeighting;$i++)
                                    ...
                                    if($questionScore!=-1)
                                    {
                                        if($i==$questionScore) echo "selected='selected'";
                                    }
                                    elseif($i==$questionWeighting) echo "selected='selected'";
                                    ?>>
									<?php echo $i; ?>
                                    </option>
									<?php } ?>
                                  ...
                                  /* My Change
                                  echo "<div class='direction:ltr; text-align:right'>";
                                  //for($i=0;$i<=$questionWeighting;$i++)
                                  for($i=$questionWeighting;$i>=0;$i--)
                                  {
                                    $check = "";
                                    if($questionScore!=-1)
                                    {
                                        if($i==$questionScore) $check = "checked='checked'";
                                    }
                                    elseif($i==$questionWeighting) $check = "checked='checked'";
                                    echo '<input type="radio" name="marks" id="marks'.$i.'" value="'.$i.'" '.$check.' />'.$i."&nbsp&nbsp&nbsp&nbsp&nbsp";
                                  }
                                  echo "</div>";
                                  */
                                   ?>
...
?>
    <tr>
        ...
        <div style="padding-right: 30px; font-weight: bold; background-color: #FFF; border-bottom: 2px dotted #a0a0a0; 
        border-top: 2px dotted #a0a0a0; line-height: 35px; box-shadow: 0px 0px 10px #a0a0a0;
        border-right: 1px solid #a0a0a0; border-left: 1px solid #a0a0a0;">
        <?php
	...
	<tr>
		...
		<?php
			if(in_array($origin, array('tracking_course','user_course'))){
				echo ' <form name="myform" id="myform" action="exercice.php?show=result&comments=update&exeid='.$id.'&test='.$test.'&emailid='.$emailId.'&origin='.$origin.'&student='.$_GET['student'].'&details=true&course='.$_GET['cidReq'].'#td'.$id.'" method="post">';
			}
			else{
				echo ' <form name="myform" id="myform" action="exercice.php?show=result&comments=update&exeid='.$id.'&test='.$test.'&emailid='.$emailId.'#td'.$id.'" method="post">';
			}
		?>
